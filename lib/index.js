...
const { scalarFn, variant, builtIn } = require('lubejs')
...
module.exports = async function connect(config) {
  ...
  options.pool = merge({}, defaultPoolOptions, {
    max: config.poolMax,
    min: config.poolMin,
...
functions.forEach(name => {
  module.exports.FUNCTIONS[name] = scalarFn(name, true)
})
...
dateParts.forEach(name => {
  module.exports.DATE_PART[name] = builtIn(name)
})
